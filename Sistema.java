...
public class Sistema {
	...
	// public void createProcess(Word[] p) {
	...
	// TODO: cuidado com paralelismo
	public int getNextPID() {
		return processos.size() + 1;
	}
	...
	public class GM {
		...
		public void free(int[] frames) {
			for (int i = 0; i < frames.length; i++) {
				...
				// for (int j = 0; j < this.frames[i].addresses.length; j++) {
				// mem.m[frames[i].addresses[j]].opc = Opcode.___;
				// mem.m[frames[i].addresses[j]].r1 = -1;
				// mem.m[frames[i].addresses[j]].r2 = -1;
				// mem.m[frames[i].addresses[j]].p = -1;
				// }
			}
		}
	}
	...
	class Process {
		...
		public void load(Word[] p) {
			int[] addresses = pcb.pages.getAddresses();
			for (int i = 0; i < p.length; i++) {
				vm.mem.m[addresses[i]] = p[i];
			}
		}

		public void free() {
			gm.free(pcb.pages.framesAddresses);
		}
	}
	...
	public Boolean criarProcesso(Word[] p) {
		...
		boolean result = gm.aloc(p.length, pages);
		if (!result) {
			return false;
		}
	...
	public static void main(String args[]) {
		...
		// // int[] out = s.gm.preAloc(10); // N existe ponteiros nessa desgraça de
		// java???
		// s.gm.aloc(10, p);
		// s.gm.aloc(14, p2);
